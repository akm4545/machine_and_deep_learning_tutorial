...
# 보통 기본값인 지니 불순도와 엔트로피 불순도가 만든 결과의 차이는 크기 않다

# 결정 트리 알고리즘은 불순도 기준을 사용해 정보 이득이 최대가 되도록 노드를 분할
# 노드를 수수하게 나눌수록 정보 이득이 커진다
# 새로운 샘플에 대해 예측할 때는 노드의 질문에 따라 트리를 이동한다
# 마지막에 도달한 노드의 클래스 비율을 보고 예측을 만든다

# 앞의 트리는 제한 없이 자라났기 때문에 훈련 세트보다 테스트 세트에서 점수가 크게 낮았다

# 가지치기
# 결정 트리 알고리즘의 가지를 치지 않으면 무작정 끝까지 자라나는 트리가 만들어진다
# 훈련 세트에는 아주 잘 맞겠지만 테스트 세트에서 점수는 그에 못 미칠 것이다
# 이를 두고 일반화가 잘 안 될 것 같다고 말한다

# 가지치기의 가장 간단한 방법은 자라날 수 있는 트리의 최대 깊이를 지정하는 것이다
# DecisionTreeClassifier 클래스의 max_depth 매개변수를 3으로 지정하여 모델을 만든다
...
# 훈련 세트의 성능은 낮아졌지만 테스트 세트의 성능은 거으 ㅣ그대로다 
# 트리 그래프 출력
plt.figure(figsize=(20, 15))
plot_tree(dt, filled=True, feature_names=['alcohol', 'sugar', 'pH'])
plt.show()

# 샘플을 어떤 클래스 비율로 나누는지 계산할 때 특성값의 스케일이 계산에 영향을 미치지 않는다
# 특성값의 스케일은 결정 트리 알고리즘에 아무런 영향을 미치지 않는다
# 따라서 표준화 전처리를 할 필요가 없다
# 이것이 결정 트리 알고리즘의 또 다른 장점 중 하나다

# 전처리 하기 전의 훈련 세트와 테스트 세트로 결정 트리 모델을 다시 훈련
dt = DecisionTreeClassifier(max_depth=3, random_state=42)
...
# 같은 트리지만 특성값을 표준점수로 바꾸지 않은 터라 이해하기 훨씬 쉽다
...
# 특성 중요도를 활용하면 결정 트리 모델을 특성 선택에 활용할 수 ㅣㅆ다
