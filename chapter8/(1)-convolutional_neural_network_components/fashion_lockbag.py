...
# 만약 흑백 이미지일 경우에는 깊이 차원이 1인 3차원 배열로 변환하여 전달한다
# ex) (28, 28) -> (28, 28, 1)

# 합성곱 층-풀링 층 다음에 다시 또 합성곱 층이 올때 이런 식으로 작동한다
# 예를 들어 첫 번째 합성곱 층의 필터 개수가 5개이고 첫 번째 풀링 층을 통과한
# 특성 맵의 크기가 (4, 4, 5)일때
# 두 번째 합성곱 층에서 필터의 너비와 높이가 각각 3이라면 이 필터의 커널 크기는 (3, 3, 5)가 된다
# 왜냐하면 입력의 깊이와 필터의 깊이는 같아야 하기 떄문이다
...
# 처음에는 간단한 기본적인 특징(직선, 곡선 등)을 찾고 층이 깊어질수록 다양하고 구체적인 
# 특징을 감지할 수 있도록 필터의 개수를 늘린다
